...
#
# NOTE:
#   - Space/Folder creation is idempotent (script reuses existing by name).
#   - Doc creation is NOT idempotent – if KB_CREATE_DOCS=true and you rerun,
#     you’ll get duplicate Docs with the same names.
#
#   For request body details, see:
#   - Create Space:  https://developer.clickup.com/reference/createspace
...
if [[ -z "${CLICKUP_API_TOKEN:-}" ]]; then
  ...
  echo "       Get it from ClickUp > Settings > Apps > API Token." >&2
  exit 1
fi

if [[ -z "${CLICKUP_TEAM_ID:-}" ]]; then
  echo "ERROR: CLICKUP_TEAM_ID is not set." >&2
  echo "       This is your Workspace/Team ID (v2 'team', v3 'workspace')." >&2
  exit 1
fi
...
#######################################
# BLUEPRINT
# Tweak this JSON to change your KB structure.
#######################################

# You can freely change names, add/remove folders or docs.
# Emojis are optional but make it nicer in the sidebar.
...
create_space() {
  ...
  # Minimal body: name + color.
  # If your Workspace requires feature flags, add a 'features' object here.
  local body
  body="$(jq -n \
    --arg name "$name" \
    --arg color "$color" \
...
create_doc() {
  ...
  # At the time of writing, the Docs API docs don’t expose a detailed example body,
  # but a minimal { "name": "<title>" } payload works for most setups.
  # If you hit 4xx here, check the official docs for any new required fields:
...
main() {
  ...
  # 2. Folders + Docs (from blueprint)
  log ""
  log "Processing Folders and Docs for Space '${SPACE_NAME}'..."

  # Iterate over folders as compact JSON objects
  jq -c '.folders[]' <<<"$KB_BLUEPRINT" | \
  while IFS= read -r folder; do
    local_folder_name="$(jq -r '.name' <<<"$folder")"

    folder_id="$(ensure_folder "$space_id" "$local_folder_name")"

    if [[ "$KB_CREATE_DOCS" == "true" ]]; then
      # iterate doc titles line-by-line to preserve spaces
      jq -r '.docs[]?' <<<"$folder" | \
  ...
  log "Knowledge Base structure setup complete."
  log "Space: ${SPACE_NAME}"
  log "Space ID: ${space_id}"
  if [[ "$KB_CREATE_DOCS" == "true" ]]; then
    log "Docs were created at Workspace level (they'll show in Docs search / Everything)."
    log "You can later move/organize them visually inside Folders in the UI if needed."
  else
    log "Docs creation is currently disabled (KB_CREATE_DOCS=false)."
    log "Set KB_CREATE_DOCS=true to auto-generate initial Docs for each folder."
  fi
}
